...
function setMovieDetails(movie) {
  ...
  const detailsData = [
    ...
    ['Date de sortie', movie.release_date ? new Date(movie.release_date).toLocaleDateString('fr-FR') : ''],
  ...
  try {
    ...
    credits.cast.slice(0, 12).forEach(person => {
      ...
      if (person.profile_path) {
        const img = el('img');
        img.src = buildImageUrl(person.profile_path, TMDB_CONFIG.IMAGE_SIZES.PROFILE.MEDIUM);
