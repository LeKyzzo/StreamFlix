...
export const tmdbApi = {
  getTrendingMovies(timeWindow = "week", page = 1) {
    return window.StreamFlix.API.fetchTmdb(`/trending/movie/${timeWindow}`, { page });
  },
  
  discoverMovies(page = 1, filters = {}) {
    return window.StreamFlix.API.fetchTmdb("/discover/movie", { page, ...filters });
  },
...
export async function fetchCollection(kind) {
  ...
  if (window.STREAMFLIX_CONFIG.USE_API) {
    try {
      const response = await fetch(`${window.STREAMFLIX_CONFIG.API_BASE}/collections/${encodeURIComponent(kind)}`);
      if (response.ok) {
...
export async function fetchMovie(id) {
  ...
  if (window.STREAMFLIX_CONFIG.USE_API) {
    try {
      const response = await fetch(`${window.STREAMFLIX_CONFIG.API_BASE}/movies/${encodeURIComponent(id)}`);
      if (response.ok) {
        const data = await response.json();
        if (data && data.id) {
          return data;
